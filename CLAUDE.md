...
- **`ge::Rect`** — `{ x, y, w, h }` float pixel rectangle on the render surface. Returned by `Context::drawSafeRect()`, `Context::uiSafeRect()`, and `Context::fullRect()`. Float fields, even though the surface is integer-sized at the framebuffer boundary, so game-side layout / animation / physics math stays in its native precision and doesn't need to cast on every read. Corner accessors `tl()`, `tr()`, `bl()`, `br()` return `ge::la::float2` so they compose with the rest of the engine's vector math.
  - **Math** (v0.8.0): `size()`, `center()`, `empty()`, `contains(point)` (half-open `[x,x+w) × [y,y+h)`), `contains(other)` (rect-fully-inside), `intersects(other)` (positive-area overlap; boundary-touching doesn't count), `intersection(other)` (overlap rect or empty), `unioned(other)` (bounding rect; absorbs empty inputs), `inset(dx, dy)` (symmetric shrink; negative expands), `inset(SafeAreaInsets)` (per-edge shrink), `translated(la::float2)`, `operator==/!=`. Half-open hit-test matches SDL/bgfx pixel coords so adjacent rects tile without overlap.
