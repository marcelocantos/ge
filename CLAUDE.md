...
- **`ge::Rect`** — `{ x, y, w, h }` float pixel rectangle on the render surface. Returned by `Context::drawSafeRect()`, `Context::uiSafeRect()`, and `Context::fullRect()`. Float fields, even though the surface is integer-sized at the framebuffer boundary, so game-side layout / animation / physics math stays in its native precision and doesn't need to cast on every read. Corner accessors `tl()`, `tr()`, `bl()`, `br()` return `ge::la::float2` so they compose with the rest of the engine's vector math.
  - **Math** (v0.8.0+): `size()`, `halfExtents()`, `center()`, `aspect()`, `empty()`, `contains(point)` (half-open `[x,x+w) × [y,y+h)`), `contains(other)` (rect-fully-inside), `intersects(other)` (positive-area overlap; boundary-touching doesn't count), `intersect(other)` (overlap rect or empty), `bbox(other)` (bounding rect; absorbs empty inputs), `inset(dx, dy)` / `inset(d)` (symmetric shrink; negative expands), `inset(SafeAreaInsets)` (per-edge shrink), `inset(la::float4)` (per-edge in CSS order `{top, right, bottom, left}`), `outset(...)` (antonym of `inset`), `translated(la::float2)`, `withOrigin(la::float2)` / `withSize(la::float2)` (copy-with-mutation), `adjusted(Rect)` (component-wise add of all four fields), `scaled(ScalingVec)` / `scaled(ScalingScalar)` (pivot scaling around a normalized rect-local center, default `{0.5, 0.5}`), `operator*(float)` / `operator/(float)`, `operator==/!=`, `Rect::centered(c, sz)`, `Rect::between(a, b)`. Half-open hit-test matches SDL/bgfx pixel coords so adjacent rects tile without overlap.
