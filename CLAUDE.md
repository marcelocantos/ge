...
- **`ge::Rect`** — `{ x, y, w, h }` float rectangle. Returned by `Context::drawSafeRect()`, `Context::uiSafeRect()`, and `Context::fullRect()` (all in screen-coord, where +y points down per SDL/bgfx). The Rect type itself is **direction-agnostic** — it doesn't bake in whether +y means up or down; that's a caller concern. Methods assume **positive w/h**; pre-normalize signed input via `r.normalized()` at the boundary (drag rects, two-corner construction, etc.) before calling other methods. Corner accessors are direction-agnostic: `x0y0()`, `x1y0()`, `x0y1()`, `x1y1()` — first index is position along x (0 = origin, 1 = far), second along y. So `x0y0()` is always the origin corner and `x1y1()` is always the far corner, regardless of the surrounding coordinate system's y-axis convention.
  - **Math** (v0.8.0+): `size()`, `halfExtents()`, `center()`, `area()` (signed `w * h`), `aspect()`, `empty()`, `contains(point)` (half-open `[x,x+w) × [y,y+h)`), `contains(other)` (rect-fully-inside), `intersects(other)` (positive-area overlap; boundary-touching doesn't count), `intersect(other)` (overlap rect or empty), `bbox(other)` (bounding rect; absorbs empty inputs), `inset(dx, dy)` / `inset(d)` (symmetric shrink; negative expands), `inset(SafeAreaInsets)` (per-edge shrink — screen-coord; assumes y-down), `inset(la::float4)` (per-edge in CSS order `{top, right, bottom, left}` — screen-coord; assumes y-down), `outset(...)` (antonym of `inset`), `translated(la::float2)`, `withOrigin(la::float2)` / `withSize(la::float2)` (copy-with-mutation), `adjusted(Rect)` (component-wise add of all four fields), `scaled(ScalingVec)` / `scaled(ScalingScalar)` (pivot scaling around a normalized rect-local center, default `{0.5, 0.5}`), `normalized()` (positive-w/h form of a possibly-signed rect; preserves the region), `operator*(float)` / `operator/(float)`, `operator==/!=`, `Rect::originSize(o, sz)` (direction-neutral factory), `Rect::centered(c, sz)`, `Rect::between(a, b)` (bbox of two points). Half-open hit-test matches SDL/bgfx pixel coords so adjacent rects tile without overlap. The `inset(SafeAreaInsets)` and `inset(la::float4)` overloads are the only methods that bake in y-down — they're inherently screen-coord operations; world-coord callers don't use them.
