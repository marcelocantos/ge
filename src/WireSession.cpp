...
struct WireSession::M {
    ...
    // Daemon connection info (set by SessionHost)
...
WireSession::WireSession(const std::string& daemonHost, uint16_t daemonPort,
    ...
    : m(std::make_unique<M>())
{
    m->daemonHost = daemonHost;
    m->daemonPort = daemonPort;
    m->sessionId = sessionId;
    m->daemonSink = std::move(sharedSink);

    SPDLOG_INFO("Session '{}': connecting to ged at {}:{}",
                sessionId, daemonHost, daemonPort);
}

void WireSession::connect() {
    std::string wirePath = "/ws/server/wire/" + m->sessionId;
    SPDLOG_INFO("Connecting wire to ged ({})...", wirePath);

    m->wsConn = connectWebSocket(m->daemonHost, m->daemonPort, wirePath);
    if (!m->wsConn) {
